...
var config = GLOBAL.config; //Get the config
GLOBAL.log = function() {
  if(config.logDebug) {
    console.log.apply(this, arguments);
  }
};
var hits = [], //These need to be available outside of count.
  cache = [],
  Counter;
...
app.get('/von-count', count);
app.post('/von-count', count);
...
//Empty those 2 sets, since if a page was being viewed when the app was shut
//off, there would be entries in them that would never be deleted otherwise.
client.zremrangebyrank('PagesCountViewedOn', 0, -1);
client.zremrangebyrank('PagesBeingRead', 0, -1);

var REDIS_CACHE_SIZE = 100; //Number of entries allowed in Redis.

function count(req, res) { //When any http request is sent to the path /von-count...
  GLOBAL.log("Recieved" + req.method);
  var origin = req.get('Origin');
  if (config.crossOriginAllowed.indexOf(origin) > -1){
    res.header("Access-Control-Allow-Origin", origin);//'*');
  }
  res.header("Access-Control-Allow-Headers", "X-Requested-With");
  res.header('Access-Control-Allow-Credentials', true);
  var hsh = req.get('Referrer');
  var sesID = req.sessionID;
  client.exists((hsh+sesID), function(err, exis) {
    if(exis == 0) {
      GLOBAL.log("Not a repeate, incrementing count");
    ...
    //entries in the set
        client.zcard('PagesCountViewedOn', function(err, lpSize) { //Get the number
    ...
    {GLOBAL.log('Repeat, not incrementing view count.');
    }
    GLOBAL.log('Setting ' + (hsh+sesID));

    client.setex((hsh+sesID), config.sessionKeepAliveSeconds,'Val', function(err, yes) {
      res.send(204);//Send an essentially empty response 
      //(204 is the code for no content).
    });
  });
}

setInterval(flushCache, 1000*config.viewCountSaveSeconds); //Set the cache to be periodically flushed
//to MongoDB.
require('./IndividualServer.js');
if(config.DISPLAY_PAGES_BEING_VIEWED) {
  require('./CountDisplayServer.js');
}
if(config.DISPLAY_RECENT_VIEW_COUNTS) {
...
function flushCache() {
  ...
  GLOBAL.log('Flushing cache');
  var cop = cache;
  cache = [];
  var hi = hits;
  hits = [];
  for(var i = 0; i < cop.length; i++) {
  GLOBAL.log('Iterating cache');
  GLOBAL.log('cop[i]', cop[i]);
  GLOBAL.log('hi[i]', hi[i]);
